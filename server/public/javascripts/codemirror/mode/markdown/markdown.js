...
CodeMirror.defineMode("markdown", function(cmCfg, modeCfg) {
  ...
  // Use `fencedCodeBlocks` to configure fenced supercolliderfiles blocks. false to
  ...
  function blockNormal(stream, state) {
    ...
    if (prevLineIsList) {
      if (state.indentationDiff >= 0) { // Continued list
        if (state.indentationDiff < 4) { // Only adjust indentation if *not* a supercolliderfiles block
    ...
    if (state.indentationDiff >= 4 && (prevLineIsIndentedCode || lineIsEmpty(state.prevLine))) {
      ...
      if (modeCfg.highlightFormatting) state.formatting = "supercolliderfiles-block";
      state.code = -1
      return getType(state);
    }
