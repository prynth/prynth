...
router.post('/setwifi', function (req, res) {
	...
	var command = 'sudo sed -i "/#prynth start/,/#prynth end/ s/\\(ssid *= *\\).*/\\1\\"'+networkname+'\\"/ ;/#prynth begin/,/#prynth end/ s/\\(password *= *\\).*/\\1\\"'+networkpass+'\\"/" /etc/wpa_supplicant/wpa_supplicant.conf';
	console.log(command);
