...
<html lang="en">
<head>
    <title>Code coverage report for DryadPlayer.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="index.html">All files</a> DryadPlayer.js
    </h1>
    <div class='clearfix'>
...
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (!rootContext.log) {
...
&nbsp;
    // default logger
    this._errorLogger = <span class="fstat-no" title="function not covered" >(m</span>sg, error) =&gt; {
      // console.log('LOGGING NOW', this.tree);
<span class="cstat-no" title="statement not covered" >      this.log.error(msg, error, error.stack);</span>
<span class="cstat-no" title="statement not covered" >      this.log.error('STATE TREE:');</span>
<span class="cstat-no" title="statement not covered" >      this.log.error(JSON.stringify(this.getDebugState(), null, 2));</span>
...
&nbsp;
    // console.log('root log', this.log, this.tree);
  }
&nbsp;
  /**
   * Set a new tree.
...
&nbsp;
  ...
  use(layer:Object) : DryadPlayer {
    this.middleware.use(layer.middleware || <span class="branch-1 cbranch-no" title="branch not covered" >[])</span>;
    (layer.classes || <span class="branch-1 cbranch-no" title="branch not covered" >[])</span>.forEach(<span class="fstat-no" title="function not covered" >(c</span>) =&gt; <span class="cstat-no" title="statement not covered" >this.addClass(c))</span>;
    return this;
  }
&nbsp;
  /**
...
&nbsp;
  ...
  getClass(className:string) : Class&lt;Dryad&gt; {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!dryadClass) {
<span class="cstat-no" title="statement not covered" >      throw new Error(`Dryad class not found: '${className}' in classes: ${Object.keys(this.classes).join(', ')}`);</span>
    }
    return dryadClass;
  }
&nbsp;
...
&nbsp;
  ...
  play(dryad:?Dryad) : Promise&lt;DryadPlayer&gt; {
    // deprec
    <span class="missing-if-branch" title="if path not taken" >I</span>if (dryad) {
<span class="cstat-no" title="statement not covered" >      this.setRoot(dryad);</span>
    }
&nbsp;
    return this.prepare()
      .then(() =&gt; {
        console.log('I am done prepare');
        return this.call('add');
      })
      .then(() =&gt; this, <span class="fstat-no" title="function not covered" >(e</span>rror) =&gt; {
        // Log the error but continue the Promise chain
<span class="cstat-no" title="statement not covered" >        this._errorLogger('Failed to play', error);</span>
...
&nbsp;
  ...
  stop() : Promise&lt;DryadPlayer&gt; {
    return this.call('remove')
      .then(() =&gt; this, <span class="fstat-no" title="function not covered" >(e</span>rror) =&gt; {
<span class="cstat-no" title="statement not covered" >        this._errorLogger('Failed to stop', error);</span>
<span class="cstat-no" title="statement not covered" >        return Promise.reject(error);</span>
      });
  }
&nbsp;
  _collectCommands(commandName:string) : CommandNode {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.tree &amp;&amp; this.tree.tree) {
      return this.tree.collectCommands(commandName, this.tree.tree, this);
    }
    // no-op
<span class="cstat-no" title="statement not covered" >    return new CommandNode({}, {}, {}, '', []);</span>
  }
&nbsp;
  /**
   ...
   */
  // _callPrepare(prepTree:CommandNode) : Promise {
  //   this._call(prepTree, 'prepareForAdd');
  //   // let prom = _.isFunction(prepTree.commands) ? Promise.resolve(prepTree.commands(prepTree.context)) : this._callAndResolveValues(prepTree.commands, prepTree.context);
    //
    // return prom.then((resolved) =&gt; {
    //   // Save resolved values into the node's context
    //   // and mark it prepare: true for debugging purposes.
    //   this.updateContext(prepTree.context, _.assign({state: {prepare: true}}, resolved));
    //   // Prepare children in series, waiting for each one to complete.
    //   // TODO: do that only when requested (eg. Properties)
    //   // otherwise prepare in parallel as before
    //   let p;
    //   prepTree.children.forEach((childPrepCommandNode) =&gt; {
    //     if (!p) {
    //       p = this._callPrepare(childPrepCommandNode);
    //     } else {
    //       p = p.then(() =&gt; this._callPrepare(childPrepCommandNode));
    //     }
    //   });
    //   return p;
    // }).then(null, (error) =&gt; {
    //   // this should update context only for error on here
    //   // if it rejects further than everything up the tree
    //   // calls this again.
    //   // I only see it twice.
    //   console.log('### _callPrepare error', error, prepTree.id);
...
&nbsp;
   ...
   *
   * TODO: examine if this works now.
   */
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  up</span>dateContext(context:Object, update:Object) : Object {
<span class="cstat-no" title="statement not covered" >    return this.tree.updateContext(context.id, update);</span>
  }
&nbsp;
  /**
  ...
  */
  // _callAndResolveValues(commands:Object, context:Object) : Promise&lt;Object&gt; {
  //   // remove
  ...
  //     let properties = this.tree.dryadProperties(context.id);
  //     return Promise.resolve(_.isFunction(value) ? value(context, properties) : value);
  //   }).then((values) =&gt; {
  //     let result = {};
  //     keys.forEach((key, i) =&gt; {
  //       result[key] = values[i];
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  ge</span>tPlayGraph() : ?Array&lt;mixed&gt; {
<span class="cstat-no" title="statement not covered" >    return this.tree.hyperscript();</span>
  }
}
&nbsp;</pre></td></tr>
</table></pre>
...
<div class='footer quiet pad2 space-top1 center small'>
  Code coverage
  generated by <a href="http://istanbul-js.org/" target="_blank">istanbul</a> at Mon Oct 17 2016 18:08:34 GMT+0200 (CEST)
</div>
</div>
