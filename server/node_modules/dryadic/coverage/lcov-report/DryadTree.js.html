...
<html lang="en">
<head>
    <title>Code coverage report for DryadTree.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="index.html">All files</a> DryadTree.js
    </h1>
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        <span class="strong">78.15% </span>
        <span class="quiet">Statements</span>
        <span class='fraction'>93/119</span>
      </div>
      <div class='fl pad1y space-right2'>
...
<span class="cstat-no" title="statement not covered" >          throw new Error(`Unsupported command ${stateTransitionName}`);</span>
      }
&nbsp;
    } catch(error) {
<span class="cstat-no" title="statement not covered" >      player.log.log(`Error during collectCommands "${stateTransitionName}" for node:\n`);</span>
<span class="cstat-no" title="statement not covered" >      player.log.log(node);</span>
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  dr</span>yadProperties(nodeId:string) : Object {
    let dryad = <span class="cstat-no" title="statement not covered" >this.dryads[nodeId];</span>
    let context = <span class="cstat-no" title="statement not covered" >this.contexts[nodeId];</span>
    // invertDryadicProperties replaces Dryads in properties with these accessor functions
<span class="cstat-no" title="statement not covered" >    return mapProperties(dryad.properties, <span class="fstat-no" title="function not covered" >(v</span>alue) : any =&gt; {</span>
<span class="cstat-no" title="statement not covered" >      return _.isFunction(value) ? value(context) : value;</span>
    });
  }
&nbsp;
  /**
...
&nbsp;
  ...
  getDebugState() : Object {
    const formatState = (s) =&gt; {
      <span class="missing-if-branch" title="else path not taken" >E</span>if (!s) {
        return s;
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (s.error) {</span>
<span class="cstat-no" title="statement not covered" >        return `ERROR: ${s.error}`;</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (s.add) {</span>
<span class="cstat-no" title="statement not covered" >        return 'running';</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (s.remove) {</span>
<span class="cstat-no" title="statement not covered" >        return 'removed';</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      if (s.prepareForAdd) {</span>
<span class="cstat-no" title="statement not covered" >        return 'prepared';</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      return `Unknown: ${JSON.stringify(s)}`;</span>
    };
&nbsp;
    const dbug = (node) =&gt; {
      let context = this.contexts[node.id];
      let state = context.hasOwnProperty('state') ? <span class="branch-0 cbranch-no" title="branch not covered" >context.state </span>: undefined;
      let r = {
        id: node.id,
...
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.tree) {
...
&nbsp;
   ...
   */
<span class="fstat-no" title="function not covered" >  hy</span>perscript() : ?Array&lt;mixed&gt; {
&nbsp;
    function <span class="fstat-no" title="function not covered" >asH(</span>node) {
      let dryad = <span class="cstat-no" title="statement not covered" >node.dryad;</span>
<span class="cstat-no" title="statement not covered" >      return [</span>
        className(dryad),
...
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (this.tree) {</span>
<span class="cstat-no" title="statement not covered" >      return asH(this.tree);</span>
    }
  }
&nbsp;
  /**
   ...
   * @returns {Object}
   */
  _createContext(dryad:Dryad, dryadId:string, parentId:?string, rootContext:Object=<span class="branch-0 cbranch-no" title="branch not covered" >{})</span> : Object {
    ...
    if (parentId) {
      ...
      <span class="missing-if-branch" title="else path not taken" >E</span>if (parentContext) {
...
&nbsp;
  ...
  _makeTree(dryad:Dryad, parentId:?string, childIndex:number|string=0, memo:Object={}) : TreeNode {
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!dryad.isDryad) {
<span class="cstat-no" title="statement not covered" >      throw new Error('Not a Dryad: ${dryad}');</span>
    }
&nbsp;
    ...
    if (memo.skipRequireParentOf === dryad) {
      ...
      if (rq) {
        if (!_.contains(memo.seenTypes, rq)) {
          ...
          <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.getClass) {
<span class="cstat-no" title="statement not covered" >            throw new Error('A getClass lookup was not provided to DryadTree and ' + dryad.constructor.name + ' needs one for requireParent()');</span>
          }
...
&nbsp;
    let makeSubgraph = (dr) =&gt; {
      ...
      if (subgraph) {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (Array.isArray(subgraph)) {
<span class="cstat-no" title="statement not covered" >          throw new Error('Dryad subgraph should return a single Dryad with children. ${dr} ${subgraph}');</span>
        }
&nbsp;
...
&nbsp;
    if (memo.skipSubgraphOf) {
      if (memo.skipSubgraphOf === dryad) {
        ...
        <span class="missing-if-branch" title="if path not taken" >I</span>if (subgraph) {
<span class="cstat-no" title="statement not covered" >          return subgraph;</span>
        }
      }
    } else {
      let subgraph = makeSubgraph(dryad);
