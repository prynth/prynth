...
function resolveOptions(configPath, commandLineOptions) {
  return new _bluebird.Promise(function (resolve, reject) {
    function ok(opts, aPath) {
      var options = _.extend(defaultOptions(), filterUndefs(opts), filterUndefs(commandLineOptions), { configPath: aPath });
    ...
    function checkPath(aPath) {
      var resolvedPath = path.resolve(untildify(aPath));
    ...
    function loadConfig(aPath) {
      try {
        ...
        reject(new _Errors2.default('Error reading config file ' + aPath + ': ' + error.mesage, {
          error: error,
          configPath: aPath
        }));
      }
    ...
    if (configPath) {
      ...
      if (localConfigPath) {
        ...
        // look in ~
        var homeDirConfigPath = checkPath(path.join(getUserHome(), '.supercollider.yaml'));
