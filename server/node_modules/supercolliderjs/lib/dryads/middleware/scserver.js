...
exports.resolveFuncs = resolveFuncs;

var _lodash = require('lodash');

var _lodash2 = _interopRequireDefault(_lodash);

var _OSCSched = require('./OSCSched');

var _OSCSched2 = _interopRequireDefault(_OSCSched);
 ...
 */
function scserver(command, context, properties) {
  if (command.scserver) {
    var cmds = resolveFuncs(command.scserver, context, properties);
    ...
    if (cmds.schedLoop) {
      ...
      if (!context.oscSched) {
        var sendFn = function sendFn(time, packets) {
          return context.scserver.send.bundle(time, packets);
        };
        context.oscSched = new _OSCSched2.default(sendFn);
      }
 ...
 */
function resolveFuncs(command, context, properties) {
  return _lodash2.default.mapValues(command, function (value) {
    return _callIfFn(value, context, properties);
  });
}
...
function _callIfFn(thing, context, properties) {
  return _lodash2.default.isFunction(thing) ? thing(context, properties) : thing;
}
