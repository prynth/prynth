...
<html lang="en">
<head>
    <title>Code coverage report for lang/internals/sclang-io.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">lang/internals</a> sclang-io.js
    </h1>
    <div class='clearfix'>
    ...
    return {
      ...
      compiling: [
        ...
        },
        {
          ...
          fn: <span class="fstat-no" title="function not covered" >(m</span>atch:RegExMatchType, text:string) =&gt; {
<span class="cstat-no" title="statement not covered" >            this.pushOutputText(text);</span>
<span class="cstat-no" title="statement not covered" >            this.processOutput();</span>
<span class="cstat-no" title="statement not covered" >            this.setState(STATES.COMPILE_ERROR);</span>
          }
        },
        {
        ...
        },
        {
          ...
          fn: <span class="fstat-no" title="function not covered" >(m</span>atch:RegExMatchType) =&gt; {
<span class="cstat-no" title="statement not covered" >            this.result.version = match[1];</span>
<span class="cstat-no" title="statement not covered" >            this.processOutput();</span>
<span class="cstat-no" title="statement not covered" >            this.setState(STATES.READY);</span>
          }
        },
        {
...
&nbsp;
                if (guid in this.calls) {
                  <span class="missing-if-branch" title="else path not taken" >E</span>if (response.type === 'Result') {
                    // anything posted during CAPTURE should be forwarded
                    // to stdout
                    stdout = this.capturing[guid].join('\n');
                    delete this.capturing[guid];
                    <span class="missing-if-branch" title="if path not taken" >I</span>if (stdout) {
<span class="cstat-no" title="statement not covered" >                      this.emit('stdout', stdout);</span>
                    }
                    this.calls[guid].resolve(obj);
                  } else {
<span class="cstat-no" title="statement not covered" >                    if (response.type === 'SyntaxError') {</span>
<span class="cstat-no" title="statement not covered" >                      stdout = this.capturing[guid].join('\n');</span>
<span class="cstat-no" title="statement not covered" >                      obj = this.parseSyntaxErrors(stdout);</span>
<span class="cstat-no" title="statement not covered" >                      delete this.capturing[guid];</span>
                    }
<span class="cstat-no" title="statement not covered" >                    this.calls[guid].reject(</span>
                  ...
                  <span class="missing-if-branch" title="if path not taken" >I</span>if (guid === '0') {
                    // out of band error
<span class="cstat-no" title="statement not covered" >                    this.emit('error', {type: response.type, error: obj});</span>
                  }
                }
...
&nbsp;
   ...
   */
  processOutput() {
    let parsed = this.parseCompileOutput((this.output || <span class="branch-1 cbranch-no" title="branch not covered" >[])</span>.join('\n'));
&nbsp;
...
&nbsp;
    while ((match = errRe.exec(rest))) {
      var file = <span class="cstat-no" title="statement not covered" >match[2];</span>
      // errors in Common library are posted as '/Common/...'
<span class="cstat-no" title="statement not covered" >      if (commonPath.exec(file)) {</span>
<span class="cstat-no" title="statement not covered" >        file = errors.dirs[0] + file;</span>
      }
<span class="cstat-no" title="statement not covered" >      errors.errors.push({</span>
        msg: match[1],
        file: file,
