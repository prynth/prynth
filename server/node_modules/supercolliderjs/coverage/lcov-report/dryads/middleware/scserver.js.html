...
<html lang="en">
<head>
    <title>Code coverage report for dryads/middleware/scserver.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">dryads/middleware</a> scserver.js
    </h1>
    <div class='clearfix'>
 ...
 */
export default function <span class="fstat-no" title="function not covered" >scserver(</span>command:Object, context:Object, properties:Object) : ?Promise&lt;Array&lt;string|number&gt;&gt; {
<span class="cstat-no" title="statement not covered" >  if (command.scserver) {</span>
    let cmds = <span class="cstat-no" title="statement not covered" >resolveThunks(command.scserver, context, properties);</span>
&nbsp;
    // send a single OSC message
<span class="cstat-no" title="statement not covered" >    if (cmds.msg) {</span>
      // TODO get default latency from context
<span class="cstat-no" title="statement not covered" >      context.scserver.send.bundle(0.03, [cmds.msg]);</span>
    }
&nbsp;
    // send an OSC bundle
<span class="cstat-no" title="statement not covered" >    if (cmds.bundle) {</span>
<span class="cstat-no" title="statement not covered" >      context.scserver.send.bundle(cmds.bundle.time, cmds.bundle.packets);</span>
    }
&nbsp;
    // schedule events using a schedLoop function
<span class="cstat-no" title="statement not covered" >    if (cmds.schedLoop) {</span>
      // initialize the scheduler on first use
<span class="cstat-no" title="statement not covered" >      if (!context.oscSched) {</span>
        const sendFn = <span class="cstat-no" title="statement not covered" ><span class="fstat-no" title="function not covered" >(t</span>ime, packets) =&gt; <span class="cstat-no" title="statement not covered" >context.scserver.send.bundle(time, packets);</span></span>
<span class="cstat-no" title="statement not covered" >        context.oscSched = new OSCSched(sendFn);</span>
      }
&nbsp;
<span class="cstat-no" title="statement not covered" >      context.oscSched.schedLoop(cmds.schedLoop, context.epoch);</span>
    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" >    if (cmds.callAndResponse) {</span>
<span class="cstat-no" title="statement not covered" >      return context.scserver.callAndResponse(cmds.callAndResponse);</span>
    }
  }
}
&nbsp;
&nbsp;
// export function resolvedCommands(command:Object, context:Object) : ?Object {
//   if (command.scserver) {
//     return resolveThunks(command.scserver, context);
//   }
// }
&nbsp;
/**
 ...
 */
export function resolveThunks(command:Object, context:Object, properties:Object) : Object {
...
function callIfFn(thing, context, properties) {
  return _.isFunction(thing) ? thing(context, properties) : <span class="branch-1 cbranch-no" title="branch not covered" >thing;</span>
}
&nbsp;</pre></td></tr>
</table></pre>
