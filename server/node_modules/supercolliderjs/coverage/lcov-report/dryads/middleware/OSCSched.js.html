...
<html lang="en">
<head>
    <title>Code coverage report for dryads/middleware/OSCSched.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
  <div class='pad1'>
    <h1>
      <a href="../../index.html">All files</a> / <a href="index.html">dryads/middleware</a> OSCSched.js
    </h1>
    <div class='clearfix'>
...
&nbsp;
   ...
   */
  constructor(sendFn:Function, latency:number=<span class="branch-0 cbranch-no" title="branch not covered" >0.05,</span> setTimeoutFn: Function=<span class="branch-0 cbranch-no" title="branch not covered" >setTimeout,</span> clearTimeoutFn:Function=<span class="branch-0 cbranch-no" title="branch not covered" >clearTimeout)</span> {
    this.sendFn = sendFn;
    this.latency = latency;
    this.setTimeout = setTimeoutFn;
    this.clearTimeout = clearTimeoutFn;
&nbsp;
    this.getNextFn = <span class="fstat-no" title="function not covered" >()</span> =&gt; <span class="cstat-no" title="statement not covered" >undefined;</span>
    this.epoch = _.now();
...
&nbsp;
  ...
  schedLoop(getNextFn:Function, epoch:?number) {
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.getNextFn) {
<span class="cstat-no" title="statement not covered" >      throw new Error('getNextFn is null');</span>
    }
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (epoch) {
      this.epoch = epoch;
    }
&nbsp;
    <span class="missing-if-branch" title="if path not taken" >I</span>if (!this.epoch) {
<span class="cstat-no" title="statement not covered" >      throw new Error(`Epoch not set: ${this.epoch}`);</span>
    }
&nbsp;
    this._schedNext();
  }
...
&nbsp;
    ...
    if (next) {
      ...
      if (delta &lt;= this.latency) {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (delta &gt; 0) {
          this._send(next.event);
        } else {
          /* eslint no-console: 0 */
          // TODO: throw EventPastDue and catch that, log it with context.log
<span class="cstat-no" title="statement not covered" >          console.warn('Event is past due. Skipping.', JSON.stringify({delta, now, event: next.event}));</span>
        }
&nbsp;
